...
void solve(std::vector<unsigned>& a, std::vector<unsigned>& b, const std::vector<std::pair<unsigned, unsigned>>& r)
{
    const size_t n = a.size(), q = r.size();

    std::vector<unsigned> s[2] = { a, b };
    for (size_t i = 0; i < 2; ++i)
        std::sort(s[i].begin(), s[i].end());

    const auto update = [&](size_t t, size_t x) {
        std::vector<unsigned>* const c[2] = { &a, &b };

        const size_t p = std::upper_bound(s[t].begin(), s[t].end(), (*c[t])[x]++) - s[t].begin() - 1;

        const unsigned old_value = std::min(s[0][p], s[1][p]);
        ++s[t][p];
        const unsigned new_value = std::min(s[0][p], s[1][p]);
    ...
    for (size_t i = 0; i < n; ++i)
        p *= std::min(s[0][i], s[1][i]);

    std::vector<unsigned> v(1 + q);
    v[0] = p;
    for (size_t i = 0; i < q; ++i) {
        const std::pair<unsigned, unsigned> d = update(r[i].first-1, r[i].second-1);
        if (d.second != d.first) {
            p /= d.first;
            p *= d.second;
        }
