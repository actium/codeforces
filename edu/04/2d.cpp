...
void solve(const std::vector<unsigned>& as, const std::vector<Query>& qs)
{
    ...
    for (const Query& q : qs) {
        if (q.t == 1) {
            st.update_point(q.i, q.v);
        } else {
            const auto predicate = [&](unsigned, unsigned range_end, const Node& node) {
                return range_end > q.l && node.max >= q.x;
            };

            const auto index = st.search(predicate);
            std::cout << static_cast<int>(index ? *index : -1) << '\n';
        }
    }
}
