...
public:
    ...
    size_t lower_bound(T target, size_t position) const
    {
        return lower_bound(target, position, 1, 0, size_);
    }
...
private:
    ...
    size_t lower_bound(T v, size_t position, size_t x, size_t lx, size_t rx) const
    {
        if (nodes_[x] < v || rx <= position)
            return size_;
        ...
        const size_t p = lower_bound(v, position, x<<1|0, lx, mx);
        return p < size_ ? p : lower_bound(v, position, x<<1|1, mx, rx);
    }
...
void solve(const std::vector<unsigned>& a, const std::vector<Query>& q)
{
    ...
    for (const Query& r : q) {
        if (r.t == 1) {
            ...
            const size_t p = st.lower_bound(r.x, r.y);
            p < a.size() ? answer(p) : no_answer();
        }
    }
}
