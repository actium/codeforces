...
struct Node {
        ...
        : count(lhs.count + rhs.count)
    {
        for (unsigned i = 0; i < 40; ++i) {
            frequencies[i] = lhs.frequencies[i] + rhs.frequencies[i];
            for (unsigned j = 0; j < i; ++j)
                count += 1ull * lhs.frequencies[i] * rhs.frequencies[j];
        }
    }
};
...
void solve(const std::vector<unsigned>& as, const std::vector<Query>& qs)
{
    ...
    for (const Query& q : qs) {
        if (q.t == 1) {
            std::cout << st.query_range(q.x - 1, q.y)->count << '\n';
        } else {
            st.update_point(q.x - 1, q.y);
        }
    }
}
