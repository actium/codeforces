...
public:
    ...
    unsigned compute(size_t lower_bound, size_t upper_bound)
    {
...
private:
    ...
    {
        if (nodes_[x].p == ~0u || lx + 1 == rx)
            return;

        nodes_[x<<1|0].p = nodes_[x<<1|0].s = nodes_[x].p;
        nodes_[x<<1|1].p = nodes_[x<<1|1].s = nodes_[x].p;
        nodes_[x].p = ~0u;
    }
    ...
    void update(size_t lb, size_t ub, unsigned v, size_t x, size_t lx, size_t rx)
    {
        ...
        nodes_[x].s = std::min(nodes_[x<<1|0].s, nodes_[x<<1|1].s);
    }

    unsigned compute(size_t lb, size_t ub, size_t x, size_t lx, size_t rx)
    {
        propagate(x, lx, rx);
        ...
        const size_t mx = (lx + rx) / 2;
        return std::min(compute(lb, ub, x<<1|0, lx, mx), compute(lb, ub, x<<1|1, mx, rx));
    }
