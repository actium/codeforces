...
class SegmentTree {
    struct Node {
        ...
        unsigned long long common_factor = 1;       // pending cumulative update for subnodes
        ...
        void update(unsigned factor)
        {
            sum = sum * factor % M;
            common_factor = common_factor * factor % M;
        }

        void propagate_updates(Node& lhs, Node& rhs)
        {
            if (common_factor != 1) {
                lhs.update(common_factor);
                rhs.update(common_factor);
                common_factor = 1;
            }
        }
