...
class SegmentTree {
    struct Node {
        ...
        unsigned common_max = 0;       // pending cumulative update for subnodes
        ...
        void update(unsigned value)
        {
            ...
            common_max = std::max(common_max, value);
        }

        void propagate_updates(Node& lhs, Node& rhs)
        {
            if (common_max != 0) {
                lhs.update(common_max);
                rhs.update(common_max);
                common_max = 0;
            }
        }
...
void solve(unsigned n, const std::vector<Query>& qs)
{
    ...
    for (const Query& q : qs) {
        if (q.t == 1) {
            ...
            std::cout << st.query(q.i, q.i + 1).max << '\n';
        }
    }
}
