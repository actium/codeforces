...
struct Node {
    ...
    void update(unsigned lower_bound, unsigned upper_bound)
    {
        height = std::max(height, lower_bound);
        height = std::min(height, upper_bound);
    }
};
...
struct Update {
    ...
    Update(unsigned type, unsigned height)
        : lower_bound(type == 2 ? 0 : height)
        , upper_bound(type == 2 ? height : ~0u)
    {}

    void aggregate(const Update& update)
    {
        if (update.lower_bound >= upper_bound) {
            lower_bound = update.lower_bound;
            upper_bound = update.lower_bound;
            return;
        }
        if (update.upper_bound <= lower_bound) {
            lower_bound = update.upper_bound;
            upper_bound = update.upper_bound;
            return;
        }

        lower_bound = std::max(lower_bound, update.lower_bound);
        upper_bound = std::min(upper_bound, update.upper_bound);
    }

    void apply(Node& target_node) const
    {
        target_node.update(lower_bound, upper_bound);
    }
};
...
void solve(unsigned n, const std::vector<Query>& qs)
{
    ...
    for (const Query& q : qs)
        st.update_range(q.l, q.r + 1, q.t, q.h);

    for (unsigned i = 0; i < n; ++i)
        std::cout << st.query_point(i)->height << '\n';
}
