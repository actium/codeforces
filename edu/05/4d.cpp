...
struct Node {
    integer range_size = 0;
    integer sum = 0;
    integer weighted_sum = 0;
    ...
    void update(int delta)
    {
        ...
        weighted_sum += delta * range_size * (range_size + 1) / 2;
    }
};
...
void solve(const std::vector<int>& as, const std::vector<Query>& qs)
{
    ...
    for (const Query& q : qs) {
        if (q.t == 1) {
            st.update_range(q.l-1, q.r, q.d);
        } else {
            std::cout << st.query_range(q.l-1, q.r)->weighted_sum << '\n';
        }
    }
}
